...
function Task({ task, tasks, }: { task: [TaskId, TaskType], tasks: [TaskId, TaskType][], }) {
  ...
  const handleTitleKeyboard = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (e.key === 'Enter') {
      actions.updateTasks([{ id: task[0], updatedFields: { title: e.currentTarget.value } }]);
  ...
  const handleDescLostFocus = (e: React.FocusEvent<HTMLTextAreaElement>) => {
    if (e.currentTarget.value !== task[1].description) {
      actions.updateTasks([{ id: task[0], updatedFields: { description: e.currentTarget.value } }]);
  ...
  const handleDeleteButton = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (task[1].status === 'deleted') {
      if (window.confirm(`By double deleting task: ${task[1].title}, you will permanently delete it. Are you sure?`)) {
        actions.hardDeleteTasks([task[0]]); // Call the hard delete function from actions with the task ID
      }
    } else {
      actions.changeStatusOfTasks([task[0]], 'deleted'); // Change the status to deleted
    }
  }
  ...
  const handleCompleteButton = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (task[1].status === 'completed' || task[1].status === 'deleted') {
      ...
      actions.changeStatusOfTasks([task[0]], 'completed'); // Change the status to completed
  ...
  const handleRestoreButton = (e: React.MouseEvent<HTMLDivElement>) => {
    e.stopPropagation(); // Prevent the click event from propagating to the parent div
    actions.changeStatusOfTasks([task[0]], task[1].previousStatus); // Restore the task to its previous status
  }
              ...
              <input type='date'
                ...
                onChange={(e) => {
                  if (!e.currentTarget.value) {
                    actions.updateTasks([{ id: task[0], updatedFields: { dueDate: undefined } }]);
                  } else {
                    actions.updateTasks([{ id: task[0], updatedFields: { dueDate: new Date(e.currentTarget.value) } }]);
